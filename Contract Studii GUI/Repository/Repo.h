...
public:
    ...
    virtual void modify(const __int64_t& position, const Disciplina& disciplina) = 0;
    virtual void remove(const __int64_t& position) = 0;
    virtual __int64_t find(const string& denumire) const = 0 ;
...
public:
	...
	void remove(const __int64_t& position) override
	{
		dict.erase(position);
		throw prob;}

	// modificarea unei masini din dict
	void modify(const __int64_t& position, const Disciplina& newD) override
	{
		dict.at(position) = newD;
		throw prob;}

	// cautarea unei masini in dict
	__int64_t find(const string& denumire) const override
	{
		for (auto& pair : dict)
			if (pair.second.getDenumire() == denumire)
				return __int64_t(pair.first);
		return -1;
	}
...
public:
    ...
    void remove(const __int64_t&) override;
    // Modificarea unei disciplini din vector
    void modify(const __int64_t& position, const Disciplina& disciplina) override;
