...
void Repo::remove(const __int64_t& position) {
    all.erase(all.begin() + position);
}

void Repo::modify(const __int64_t& position, const Disciplina& disciplina) {
    all.at(position) = disciplina;
}


__int64_t Repo::find(const string& denumire)const {
    auto x = find_if(all.begin(), all.end(), [denumire](const Disciplina& d) {return d.getDenumire() == denumire; });
    const __int64_t y = distance(all.begin(), x);
    if (y - all.size() == 0)
        return -1;
    return y;
}
...
void testRepo()
{
    ...
    __int64_t position = repo.find(testDisciplina3.getDenumire());
