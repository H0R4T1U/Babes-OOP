...
void DisciplinaConsole::addToContractList()
{
	...
	try
	{
		contractService.addDisciplina(disciplina);
	}
	catch (const Error& err)
	{
		cout << err.getMessage();
...
void DisciplinaConsole::generateContractList()
{
	...
	try
	{
		contractService.populateContract(number);
	}
	catch (const Error& err)
	{
		cout << err.getMessage();
...
void DisciplinaConsole::start()
{
	...
	while (true)
	{
		...
		else if (option == 11)
		{
			...
			while (1)
			{
				cout << "Sunt " << contractService.contractSize() << " discipline in contract\n";
				printAllDiscipline(contractService.contractList());
				printContractMenu();
				cin >> contractOption;
				if (contractOption == 1)
					emptyContractList();
