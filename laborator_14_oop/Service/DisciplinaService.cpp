...
void DisciplinaService::deleteDisciplina(const string& denumire) {
	const __int64_t position = rep.find(denumire);
	if (position < 0)
...
void DisciplinaService::modifyDisciplina(const string& denumire, const string& new_Denumire, int ore, const string& tip, const string& profesor) {
	__int64_t position = rep.find(denumire);
	if (position < 0)
		throw InexistentDisciplinaError("disciplina nu exista in baza de date!\n");
...
{
	ds.addDisciplina("ASC", 31, "Frecventa", "Vancea");
	ds.addDisciplina("FP",32 , "Fara Frecventa", "Istvan");
	ds.addDisciplina("OS",33 , "Frecventa", "Boian");
	ds.addDisciplina("OOP", 34, "Fara Frecventa", "Istvan");
	ds.addDisciplina("RETELE", 35, "Frecventa", "Bufnea");
}
...
{
	ofstream out;
	out.open(fileName+".csv", ios::trunc);
	if (!out.is_open())
...
{
	ofstream out;
	out.open(fileName+".html", ios::trunc);
	if (!out.is_open())
		throw InvalidFileError("Fisierul nu a putut fi deschis!\n");
...
const vector<Disciplina> DisciplinaService::getContractList()
{
